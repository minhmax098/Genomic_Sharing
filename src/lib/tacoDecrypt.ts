...
export async function tacoDecryptToBytes(params: {
    ...
    // 1. Provider wallet (Sepolia) to sign
    const web3Provider = new Web3Provider(
        window.ethereum as unknown as ExternalProvider
    );

    // 2. Provider connect Amoy network to TACo receve decrypt request
    ...
    // authProvider use web3Provider cuz MetaMask signer
    ...
    // Transfer tacoNodeProvider into decrypt
